...
func MemoryFilterBy(c echo.Context) error {
	...
	if Ftag == "" && Fdesc == "" {
		...
		if err := db.Joins("JOIN memory_tag on memory.id = memory_tag.memory_id").Joins("JOIN tag on tag.id = memory_tag.tag_id").Where("user_id = ? and memory.description LIKE ?", currentUser.UserID, Fdesc+"%").Preload("Pictures").Preload("MemoriesTags").Preload("MemoriesTags.Tag").Distinct().Find(&memories).Error; err != nil {
